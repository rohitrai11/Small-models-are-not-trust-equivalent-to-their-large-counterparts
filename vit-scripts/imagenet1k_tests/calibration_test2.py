#this script computes metrics using quantile binning for the ImageNet-1K validation set, using the non-streaming version of the dataset to ensure stable binning. It also generates a reliability diagram with confidence intervals for each bin. The script is designed to run on multiple ViT models and saves the results in a structured format for easy analysis.
...
MODEL_CONFIGS = {
    "vit_base": {
        "model_path": "/home/gpuuser0/gpuuser0_a/yatika-btp/from-95-GB/yatika/vit-scripts/finetune_vit_imagenet/vit_head_base_224",
        "processor_path": "google/vit-base-patch16-224-in21k"
    },
    "vit_large": {
        "model_path": "/home/gpuuser0/gpuuser0_a/yatika-btp/from-95-GB/yatika/vit-scripts/finetune_vit_imagenet/fine_tune_large/vit_head_large_224",
        "processor_path": "google/vit-large-patch16-224-in21k"
    },
    "vit_huge": {
        "model_path": "/home/gpuuser0/gpuuser0_a/yatika-btp/from-95-GB/yatika/vit-scripts/finetune_vit_imagenet/fine-tune-huge/vit_head_only_huge_224",
        "processor_path": "google/vit-huge-patch14-224-in21k"
    }
}

BASE_OUTPUT_DIR = "./Predictions_2/ViT-ImageNet/"
...
# METRICS (UPDATED)
# =========================
def compute_metrics(probs, labels, output_dir, num_bins=10, taskname="ImageNet"):
    ...
    softmax_bins_dict = {
        bin_label(i): int(bin_counts[i]) for i in range(actual_num_bins)
    }

    true_positive_dict = {
        bin_label(i): (
            float(true_positive_rates[i]) if not np.isnan(true_positive_rates[i]) else None
        )
    ...
    avg_conf_dict = {
        bin_label(i): (
            float(avg_confidences[i]) if not np.isnan(avg_confidences[i]) else None
        )
        for i in range(actual_num_bins)
    }
